...
import { Path, PatternPath, MatrixPath, MatrixPathPattern, PathElementString, PathElementPattern } from './path';
...
test('parses a path with wildcards', () => {
    let path = PatternPath.parse('ab/*/def?.txt');
...
test('roundtrips a path with wildcards', ()=>{
    let path = PatternPath.parse('ab/*/def?.txt').toString();
    expect(path).toEqual('ab/*/def?.txt');
});

test('matches a path with wildcards', () => {
    let pattern = PatternPath.parse('ab/*/def?.txt');
...
test('compares pattern paths', () => {
    let path1 = PatternPath.parse('a/b/c');
    let path2 = PatternPath.parse('a/b/c');
    let path3 = PatternPath.parse('*');
    let path4 = PatternPath.parse('?');
    let path5 = PatternPath.parse('*');
    expect(path1.equals(path2)).toBe(true);
    expect(path1.equals(path3)).toBe(false);
    expect(path1.equals(path4)).toBe(false);
...
test('parses a matrix path', ()=> {
    let path = MatrixPath.parse('abc;version=1/def;version=2;color=green/xyz');
...
test('roundtrips a matrix path', ()=> {
    let path = MatrixPath.parse('abc;version=1/def;version=2;color=green/xyz').toString();
...
test('compares matrix paths', () => {
    let path1 = MatrixPath.parse('a/b;v=2/c');
    let path2 = MatrixPath.parse('a/b;v=2/c');
    let path3 = MatrixPath.parse('a/b;v=3/c');
    let path4 = MatrixPath.parse('a/b;v=2/d');
    expect(path1.equals(path2)).toBe(true);
    expect(path1.equals(path3)).toBe(false);
    expect(path1.equals(path4)).toBe(false);
});
...
test('parses a matrix pattern', ()=> {
    let path = MatrixPathPattern.parse('abc;version=?/def*;version=2;color=gr*/xyz');
...
test('roundtrips a matrix pattern', ()=> {
    let path = MatrixPathPattern.parse('abc;version=?/def*;version=2;color=gr*/xyz').toString();
    expect(path).toEqual('abc;version=?/def*;version=2;color=gr*/xyz');
});

test('matches a matrix pattern', ()=> {
    let pattern : MatrixPathPattern = MatrixPathPattern.parse('abc;version=?/def*;version=2;color=gr*/xyz');
    let path1 : MatrixPath = MatrixPath.parse('abc;version=1/defghi;version=2;color=grey/xyz');
    let path2 : MatrixPath= MatrixPath.parse('abc;version=12/defghi;version=2;color=grey/xyz');
    let path3 : MatrixPath = MatrixPath.parse('abc;version=1/defghi;version=2;color=black/xyz');
    let path4 : MatrixPath = MatrixPath.parse('abc;version=1;color=puce/defghi;version=2;color=grey/xyz');
...
test('Path.of is polymorphic', ()=> {
    expect(Path.of('a','b')).toBeInstanceOf(Path);
    expect(PatternPath.of(Pattern.of('a'), Pattern.of('b'))).toBeInstanceOf(PatternPath);
    expect(MatrixPath.of(new PathElementString('a'), new PathElementString('b'))).toBeInstanceOf(MatrixPath);
    expect(MatrixPathPattern.of(new PathElementPattern(Pattern.of('a')), new PathElementPattern(Pattern.of('a')))).toBeInstanceOf(MatrixPathPattern);
})